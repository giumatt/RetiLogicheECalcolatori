...
escursione_termica:
    ...
    mov ecx, [ebp+W]
    mov di, [ebp+c]
    mov bx, [eax+2]         ;Temp max prima città
    sub bx, [eax]           ;Escursione termica prima città
    add edx, 4              ;Mi sposto sulla temp minima della seconda città
    mov si, bx              ;si = esc max corrente
    mov ax, bx              ;dx conterrà la somma delle esc termiche delle città
...
.final:
    ...
    jmp .ciclo
.fine_ciclo:
    mov [ecx], si           ;spostamento del max trovato il W[0]
    xor dx, dx
    mov edi, [ebp+c]
    shr di, 1
    idiv di
